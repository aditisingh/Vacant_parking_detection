...
int main( int argc, char** argv )
{
	...
	if( argc != 3)
    {
     cout <<" Usage: prog_exec cascade_file_path test_image_path" << endl;
     return -1;
    }

	String car_cascade_name = argv[1];
	...
	{
		rectangle(frame, Point(cars[i].x, cars[i].y), Point(cars[i].x + cars[i].width, cars[i].y + cars[i].height), Scalar(255, 0, 0), 2);
			...
			if(occupied_flag)
				{
					...
					cout<<"Detected: Occupied"<<endl;
					// circle(frame, Point(x_coord,y_coord), 3,Scalar(0,0,255), -1,8,0);
				}
				else{
					cout<<"Detected: Vacant"<<endl;
				}
				if(Occupied[i])
						cout<<"Actually: Occupied"<<endl;
					else
						cout<<"Actually: Vacant"<<endl;
	...
	// cout<<endl<<"Spaces :"<<SpaceNos<<" occupied total: "<<total_occupied<<", detected: "<<occupied_count<<" ,percentage: "<<100.0*occupied_count/total_occupied<<endl;
	// for (int i = 0; i < SpaceNos; i++)
	// {
	// 	cout<<Occupied[i]<<" "<<Detected[i]<<endl;
	// }
	cout<<endl<<"True Positives: "<<occupied_count<<" , False Negatives: "<<total_occupied-occupied_count<<", Accuracy: "<<100.0*occupied_count/total_occupied<<"%%"<<endl;

	namedWindow("Frame", WINDOW_NORMAL);
    imshow("Frame",frame);
    cout<<"Results saved as result.jpg"<<endl;
    cout<<argv[2]<<endl;
